...
def find_average_contacts_in_subfolders(root_path):
    ...
    for folder_name in os.listdir(root_path):
        ...
        if os.path.isdir(folder_path):
            file_path = os.path.join(folder_path, "all_contacts.pkl")
            if not os.path.isfile(file_path):
                file_path = os.path.join(folder_path, "1", "all_contacts.pkl")
...
if __name__ == "__main__":
    data, data2 = find_average_contacts_in_subfolders(PosixPath(os.path.abspath(__file__)).parent.parent / "output")
